...
export default class SensenPluginAssets{
    ...
    static async Script(url: string, lazy?: boolean){

        return new Promise<HTMLScriptElement>((resolved, rejected) => {
            
            let get = document.querySelector(`script[src='${ url }']`);


            if(get){ get.parentNode?.removeChild(get); }
    ...
    static async CSS(url: string, lazy?: boolean){

        return new Promise<HTMLStyleElement>((resolved, rejected) => {
            ...
            const e = (document.createElement('style')) as HTMLStyleElement;
    ...
    static async Image(url: string, lazy?: boolean){
